...
5. **Tela de execução de treino:** peso e reps sempre visíveis e pré-preenchidos com o valor da última vez que o exercício foi feito. *Esta regra foi PARCIALMENTE ALTERADA por decisão explícita do usuário:* **RIR (chips 0-4) e o badge de tipo de série** (normal / A=aquecimento / P=preparatória / F=falha, alternado tocando no número da série) são **SEMPRE VISÍVEIS** — o texto original mandava esconder os dois. Continuam atrás do "mais opções": **RPE** e as demais técnicas avançadas (drop-set, superset, myo-reps, rest-pause etc.). Regra prática: RIR 0 **é** falha, não são dois dados diferentes.
   ...
   **5.1. NENHUM campo de peso/reps é somente-leitura na execução — nem no treino do coach, nem no aquecimento/feeder, nem depois do ✓.** *(Corrigido em 2026-08-07 a partir de bug relatado pelo usuário: o kg do aquecimento vinha travado no valor calculado e ele não conseguia registrar que tinha aquecido com mais peso — "não consigo colocar valor maior que 10".)* O que o motor calcula (25% e 50% da carga de trabalho) é **sugestão pré-preenchida**, não prescrição fechada: o registro tem que ser o que a pessoa levantou de verdade, senão o histórico mente e a regra 4 cai junto. Editar uma série **já confirmada** salva a correção de verdade no servidor (PATCH na própria linha do log, via `logId` guardado ao confirmar) — mudar só o número na tela seria pior que travar. O que continua fixo é a **estrutura** decidida pelo motor: tipo da série (badge A/P), quantidade de séries e a técnica avançada prescrita.
...
- **DÁ PRA MANDAR POR OTA (`eas update`)** — *(regra CORRIGIDA em 2026-08-07: eu vinha dizendo "precisa de build novo" pra toda mudança de `mobile/`, e isso é FALSO.)* O app tem `expo-updates` configurado e em uso: canal `production`, `runtimeVersion` pela política `appVersion`. **Mudança que é só JS/TS (telas, lógica de componente, chamada de API) chega em quem já tem o app instalado com `eas update --channel production`, sem build e sem loja** — o app baixa na abertura e aplica na abertura seguinte. É o caminho PADRÃO pra correção de bug de tela. Só vale pra quem tem um build com o MESMO `runtimeVersion` (hoje `1.0.0`) instalado.
- **PRECISA DE NOVO BUILD DO APP (.aab/.ipa)** — só quando a mudança é **NATIVA**: dependência nova com código nativo, plugin do Expo, permissão, ícone/splash, mudança em `app.json` que afete o binário, ou bump de `expo.version` (que muda o `runtimeVersion` e corta o OTA dos instalados). **Não é o caso de mudança só de tela** — antes de anunciar "precisa de build", verificar se a alteração é realmente nativa.
